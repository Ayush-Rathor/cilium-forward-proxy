...
static __always_inline int lxc_redirect_to_host(
	struct __ctx_buff *ctx, __u32 src_sec_identity, __be16 proto,
	struct trace_ctx *trace)
{
	send_trace_notify(
		ctx, TRACE_TO_HOST, src_sec_identity, HOST_ID, TRACE_EP_ID_UNKNOWN,
		CONFIG(cilium_net_ifindex), trace->reason, trace->monitor, proto);
 ...
 */
#if !defined(ENABLE_SOCKET_LB_FULL) || defined(ENABLE_SOCKET_LB_HOST_ONLY) || \
	defined(ENABLE_L7_LB) || defined(ENABLE_SCTP) ||                      \
...
static __always_inline void lb4_ctx_restore_state(
	...
	__u32 *cluster_id __maybe_unused, bool clear)
{
	...
	*proxy_port =
		clear ? (ctx_load_and_clear_meta(ctx, CB_PROXY_MAGIC) >> 16) :
	...
	__u32 cluster_id)
{
	...
	ctx_store_meta(ctx, CB_CT_STATE,
		       (__u32)state->rev_nat_index << 16 | state->loopback);
...
#  endif /* ENABLE_L7_LB */
		...
		ret = lb4_local(get_ct_map4(&tuple), ctx, fraginfo, l4_off, &key,
				&tuple, svc, &ct_state_new, &backend, ext_err);
...
#  ifdef SERVICE_NO_BACKEND_RESPONSE
				ret = tail_call_internal(
					ctx, CILIUM_CALL_IPV4_NO_SERVICE, ext_err);
#  endif
			}
			return ret;
		}
		...
		if (tuple.saddr == backend->address) {
			if (CONFIG(enable_lrp)) {
				__net_cookie netns_cookie =
					CONFIG(endpoint_netns_cookie);

				if (netns_cookie > 0 &&
				    unlikely(lb4_svc_is_localredirect(svc)) &&
				    lrp_v4_skip_xlate_from_ctx_to_svc(
					    netns_cookie, tuple.daddr, tuple.sport))
					goto skip_service_lookup;
			}
...
#  endif
		...
		ret = lb4_dnat_request(
			ctx, backend, ETH_HLEN, fraginfo, l4_off, &tuple,
			ct_state_new.loopback);
		if (IS_ERR(ret))
			return ret;
	}
skip_service_lookup:
...
static __always_inline int
__per_packet_lb_svc_xlate_6(void *ctx, struct ipv6hdr *ip6, __s8 *ext_err)
{
...
#  endif /* ENABLE_L7_LB */
		...
		ret = lb6_local(get_ct_map6(&tuple), ctx, fraginfo, l4_off, &key,
				&tuple, svc, &ct_state_new, &backend, ext_err);
...
#  ifdef SERVICE_NO_BACKEND_RESPONSE
				ret = tail_call_internal(
					ctx, CILIUM_CALL_IPV6_NO_SERVICE, ext_err);
#  endif
			}
			return ret;
		}
		...
		if (ipv6_addr_equals(&tuple.saddr, &backend->address)) {
			if (CONFIG(enable_lrp)) {
				...
				if (netns_cookie > 0 &&
				    unlikely(lb6_svc_is_localredirect(svc)) &&
				    lrp_v6_skip_xlate_from_ctx_to_svc(
					    netns_cookie, tuple.daddr, tuple.sport))
					goto skip_service_lookup;
			}
...
static __always_inline int drop_for_direction(
	struct __ctx_buff *ctx, enum ct_dir dir, int reason, __s8 ext_err)
{
	__u32 dst = 0;
	__u32 dst_id = 0;
...
# endif
	...
	return send_drop_notify_ext(
		ctx, src_label, dst, dst_id, reason, ext_err, m_dir);
}
#endif /* ENABLE_IPV4 || ENABLE_IPV6 */

#define TAIL_CT_LOOKUP4(ID, NAME, DIR, CONDITION, TARGET_ID, TARGET_NAME)              \
	__declare_tail(ID) static __always_inline int                                  \
	 ...
	 */ \
		if (is_defined(ENABLE_PER_PACKET_LB) && DIR == CT_EGRESS) {            \
			...
			lb4_ctx_restore_state(                                         \
				ctx, &ct_state_new, &proxy_port, &cluster_id,          \
				false);                                                \
			...
			if (is_defined(ENABLE_L7_LB) &&                                \
				...
				scope = SCOPE_FORWARD;                                 \
                                                                                       ...
                                                                                       \
		ct_buffer->ret = ct_lookup4(                                           \
			map, tuple, ctx, ip4, ct_buffer->l4_off, DIR, scope,           \
...
#define TAIL_CT_LOOKUP6(ID, NAME, DIR, CONDITION, TARGET_ID, TARGET_NAME)      \
	...
	{                                                                      \
		...
		ipv6_addr_copy(&tuple->saddr, (union v6addr *)&ip6->saddr);    \
                                                                               \
		hdrlen = ipv6_hdrlen_with_fraginfo(                            \
			ctx, &tuple->nexthdr, &ct_buffer->fraginfo);           \
		if (hdrlen < 0)                                                \
		...
		if (is_defined(ENABLE_PER_PACKET_LB) && DIR == CT_EGRESS) {    \
			...
			lb6_ctx_restore_state(                                 \
				ctx, &ct_state_new, &proxy_port, false);       \
			if (ct_state_new.rev_nat_index)                        \
				scope = SCOPE_FORWARD;                         \
			if (is_defined(ENABLE_L7_LB) && proxy_port)            \
				scope = SCOPE_FORWARD;                         \
			if (is_defined(ENABLE_L7_LB) &&                        \
				...
				scope = SCOPE_FORWARD;                         \
                                                                               ...
                                                                               \
		ct_buffer->ret = ct_lookup6(                                   \
			get_ct_map6(tuple), tuple, ctx, ip6,                   \
			ct_buffer->fraginfo, ct_buffer->l4_off, DIR, scope,    \
			ct_state, &ct_buffer->monitor);                        \
		if (ct_buffer->ret < 0)                                        \
			return drop_for_direction(                             \
				ctx, DIR, ct_buffer->ret, ext_err);            \
...
#ifdef ENABLE_IPV6
static __always_inline int ipv6_forward_to_destination(
	struct __ctx_buff *ctx, struct ipv6hdr *ip6,
	struct ipv6_ct_tuple *tuple, const __u32 dst_sec_identity,
	const struct ct_state *ct_state, enum ct_status ct_status,
	const struct remote_endpoint_info *info __maybe_unused,
	bool skip_tunnel __maybe_unused, bool hairpin_flow, bool from_l7lb,
	__u16 proxy_port, struct trace_ctx *trace, __s8 *ext_err)
{
		...
		if (vrf_id) {
			...
			if (sid) {
				...
				return tail_call_internal(
					ctx, CILIUM_CALL_SRV6_ENCAP, ext_err);
			}
		}
	}
# endif /* ENABLE_SRV6 */
	...
	if (!from_l7lb && proxy_port > 0) {
		...
		send_trace_notify(
			ctx, TRACE_TO_PROXY, SECLABEL_IPV6, UNKNOWN_ID,
...
# if defined(ENABLE_HOST_ROUTING) || defined(ENABLE_ROUTING)
			if (ep->flags & ENDPOINT_MASK_HOST_DELIVERY) {
				if (is_defined(ENABLE_ROUTING) &&
					...
					return lxc_redirect_to_host(
						ctx, SECLABEL_IPV6,
						bpf_htons(ETH_P_IPV6), trace);
...
# endif /* ENABLE_HOST_ROUTING || ENABLE_ROUTING */
			...
			return ipv6_local_delivery(
				ctx, ETH_HLEN, SECLABEL_IPV6, MARK_MAGIC_IDENTITY,
				ep, METRIC_EGRESS, from_l7lb, false);
		}
	}
...
#  endif /* !ENABLE_NODEPORT && ENABLE_HOST_FIREWALL */
		...
		if (info && info->flag_has_tunnel_ep)
			return encap_and_redirect_lxc(
				ctx, info, SECLABEL_IPV6, dst_sec_identity,
				trace, bpf_htons(ETH_P_IPV6));
	}
# endif
	if (is_defined(ENABLE_HOST_ROUTING)) {
		...
		ret = fib_redirect_v6(
			ctx, ETH_HLEN, ip6, false, false, ext_err, &oif, tbid);
		/*
		...
		case CTX_ACT_REDIRECT:
			send_trace_notify(
				ctx, TRACE_TO_NETWORK, SECLABEL_IPV6,
...
# ifndef ENABLE_ROUTING
		...
		if (from_l7lb && ctx_get_ifindex(ctx) != CONFIG(cilium_host_ifindex)) return ctx_redirect(
			ctx, ctx_get_ifindex(ctx), 0);
# endif /* !ENABLE_ROUTING */

	send_trace_notify(
		ctx, TRACE_TO_STACK, SECLABEL_IPV6, dst_sec_identity,
		TRACE_EP_ID_UNKNOWN, TRACE_IFINDEX_UNKNOWN, trace->reason,
...
# if defined(ENABLE_L7_LB)
		...
		if (proxy_port > 0) {
			...
			cilium_dbg3(ctx, DBG_L7_LB, tuple->daddr.p4,
				    tuple->saddr.p4, bpf_ntohs(proxy_port));
			break;
		}
# endif /* ENABLE_L7_LB */
		 ...
		 */
		verdict = policy_can_egress6(
			ctx, tuple, l4_off, SECLABEL_IPV6, *dst_sec_identity,
		...
		if (verdict == DROP_POLICY_AUTH_REQUIRED) {
			...
			verdict = auth_lookup(
				ctx, SECLABEL_IPV6, *dst_sec_identity,
		...
		if (verdict != CTX_ACT_OK || ct_status != CT_ESTABLISHED) {
			send_policy_verdict_notify(
				...
				tuple->nexthdr, POLICY_EGRESS, 1, verdict,
				proxy_port, policy_match_type, audited,
				auth_type, cookie);
		}
		...
		if (verdict != CTX_ACT_OK) {
			...
			if (CONFIG(policy_deny_response_enabled) &&
			    (verdict == DROP_POLICY ||
			     verdict == DROP_POLICY_DENY)) {
				ctx_store_meta(ctx, CB_VERDICT, verdict);
				return tail_call_internal(
					ctx, CILIUM_CALL_IPV6_POLICY_DENIED,
					ext_err);
			}
			return verdict;
		}
...
ct_recreate6:
		...
		ret = ct_create6(get_ct_map6(tuple), &cilium_ct_any6_global,
				 tuple, ctx, CT_EGRESS, &ct_state_new, ext_err);
		if (IS_ERR(ret))
			return ret;
	...
	case CT_ESTABLISHED:
		...
		/* See comment in handle_ipv4_from_lxc(). */
		ct_state_new.proxy_redirect = proxy_port > 0;
		if (unlikely(ct_state->proxy_redirect !=
			     ct_state_new.proxy_redirect))
			goto ct_recreate6;
		break;
		...
		if (ct_state->node_port && lb_is_svc_proto(tuple->nexthdr)) {
			send_trace_notify(
				...
				*dst_sec_identity, TRACE_EP_ID_UNKNOWN,
				...
				trace.monitor, bpf_htons(ETH_P_IPV6));
			return tail_call_internal(
				ctx, CILIUM_CALL_IPV6_NODEPORT_REVNAT_EGRESS,
				ext_err);
		}
# endif /* ENABLE_NODEPORT */
		break;
	...
	return ipv6_forward_to_destination(
		ctx, ip6, tuple, *dst_sec_identity, ct_state, ct_status, info,
		skip_tunnel, hairpin_flow, from_l7lb, proxy_port, &trace, ext_err);
}

__declare_tail(CILIUM_CALL_IPV6_FROM_LXC_CONT) static __always_inline
	int tail_handle_ipv6_cont(struct __ctx_buff *ctx)
{
	...
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, SECLABEL_IPV6, dst_sec_identity,
			TRACE_EP_ID_UNKNOWN, ret, ext_err, METRIC_EGRESS);

	return ret;
}

TAIL_CT_LOOKUP6(
	CILIUM_CALL_IPV6_CT_EGRESS, tail_ipv6_ct_egress, CT_EGRESS,
	is_defined(ENABLE_PER_PACKET_LB), CILIUM_CALL_IPV6_FROM_LXC_CONT,
	tail_handle_ipv6_cont)

static __always_inline int
__tail_handle_ipv6(struct __ctx_buff *ctx, __s8 *ext_err)
{
	void *data, *data_end;
	struct ipv6hdr *ip6;
	bool from_l7lb = false;
...
__declare_tail(CILIUM_CALL_IPV6_FROM_LXC) int tail_handle_ipv6(
	struct __ctx_buff *ctx)
{
	...
	if (IS_ERR(ret))
		return send_drop_notify_error_ext(
			ctx, SECLABEL_IPV6, ret, ext_err, METRIC_EGRESS);
	return ret;
}
#endif /* ENABLE_IPV6 */
...
# endif /* ENABLE_SRV6 */
	...
	if (!from_l7lb && proxy_port > 0) {
		/* Trace the packet before it is forwarded to proxy */
		send_trace_notify(
			ctx, TRACE_TO_PROXY, SECLABEL_IPV4, UNKNOWN_ID,
			bpf_ntohs(proxy_port), TRACE_IFINDEX_UNKNOWN,
...
# if defined(ENABLE_VTEP)
	{
		...
		if (vtep && vtep->vtep_mac && vtep->tunnel_endpoint) {
			...
			return __encap_and_redirect_with_nodeid(
				ctx, &fake_info, SECLABEL_IPV4, WORLD_IPV4_ID,
				WORLD_IPV4_ID, trace, bpf_htons(ETH_P_IP));
		}
	}
# endif
...
# endif /* TUNNEL_MODE */
	...
	if (is_defined(ENABLE_HOST_ROUTING)) {
		...
		ret = fib_redirect_v4(
			ctx, ETH_HLEN, ip4, false, false, ext_err, &oif, tbid);
		/*
		 * if the endpoint is configured with an explicit table id,
		 * be strict and drop the traffic if we are not redirected.
		 */
		...
		case CTX_ACT_REDIRECT:
			send_trace_notify(
				ctx, TRACE_TO_NETWORK, SECLABEL_IPV4,
				dst_sec_identity, TRACE_EP_ID_UNKNOWN, oif,
				trace->reason, trace->monitor,
				bpf_htons(ETH_P_IP));
			return ret;
...
# endif /* !ENABLE_ROUTING */
	...
	send_trace_notify(
		ctx, TRACE_TO_STACK, SECLABEL_IPV4, dst_sec_identity,
		TRACE_EP_ID_UNKNOWN, TRACE_IFINDEX_UNKNOWN, trace->reason,
...
# endif	 /* ENABLE_PER_PACKET_LB */
	...
	if (CONFIG(hybrid_routing_enabled)) {
		...
		same_subnet_id =
			(src_subnet_id == dst_subnet_id) && (src_subnet_id != 0);
	}
...
# endif /* ENABLE_L7_LB */
		...
		verdict = policy_can_egress4(
			ctx, tuple, l4_off, SECLABEL_IPV4, *dst_sec_identity,
			&policy_match_type, &audited, ext_err, &proxy_port,
			&cookie);
		...
		if (verdict != CTX_ACT_OK) {
			...
			if (CONFIG(policy_deny_response_enabled) &&
				...
				ctx_store_meta(ctx, CB_VERDICT, verdict);
				return tail_call_internal(
					ctx, CILIUM_CALL_IPV4_POLICY_DENIED,
					ext_err);
			}
			return verdict;
		}
...
ct_recreate4:
		...
		ret = ct_create4(ct_map, ct_related_map, tuple, ctx, CT_EGRESS,
		...
		if (ct_state->node_port && lb_is_svc_proto(tuple->nexthdr)) {
			send_trace_notify(
				...
				*dst_sec_identity, TRACE_EP_ID_UNKNOWN,
				...
				trace.monitor, bpf_htons(ETH_P_IP));
			return tail_call_internal(
				ctx, CILIUM_CALL_IPV4_NODEPORT_REVNAT, ext_err);
		}
# endif /* ENABLE_NODEPORT */
	...
	return ipv4_forward_to_destination(
		ctx, ip4, tuple, *dst_sec_identity, ct_state, ct_status, info,
		skip_tunnel, hairpin_flow, from_l7lb, proxy_port, cluster_id,
		&trace, ext_err);
}

__declare_tail(CILIUM_CALL_IPV4_FROM_LXC_CONT) static __always_inline
...
TAIL_CT_LOOKUP4(
	CILIUM_CALL_IPV4_CT_EGRESS, tail_ipv4_ct_egress, CT_EGRESS,
	is_defined(ENABLE_PER_PACKET_LB), CILIUM_CALL_IPV4_FROM_LXC_CONT,
	tail_handle_ipv4_cont)

static __always_inline int
__tail_handle_ipv4(struct __ctx_buff *ctx, __s8 *ext_err __maybe_unused)
{
	void *data, *data_end;
	struct iphdr *ip4;
	bool from_l7lb = false;
...
__declare_tail(CILIUM_CALL_IPV4_FROM_LXC) int tail_handle_ipv4(
	struct __ctx_buff *ctx)
{
	__s8 ext_err = 0;
	...
	if (IS_ERR(ret))
		return send_drop_notify_error_ext(
			ctx, SECLABEL_IPV4, ret, ext_err, METRIC_EGRESS);
	return ret;
}
...
__section_entry int cil_from_container(struct __ctx_buff *ctx)
{
	...
	send_trace_notify(
		ctx, TRACE_FROM_LXC, sec_label, UNKNOWN_ID, TRACE_EP_ID_UNKNOWN,
		TRACE_IFINDEX_UNKNOWN, TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN,
		proto);
...
out:
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, sec_label, UNKNOWN_ID, TRACE_EP_ID_UNKNOWN, ret,
	    ...
	    bool from_tunnel)
{
	...
	is_untracked_fragment =
		!CONFIG(enable_ipv6_fragments) && ipfrag_is_fragment(fraginfo);
	...
	case CT_RELATED:
		...
		if (unlikely(ct_state->rev_nat_index || ct_state->nat_port)) {
			...
			ret2 = lb6_rev_nat(
				ctx, l4_off, ct_state->rev_nat_index,
...
# endif /* ENABLE_PER_PACKET_LB */
		...
		verdict = policy_can_ingress6(
			ctx, tuple, l4_off, is_untracked_fragment, src_label,
			SECLABEL_IPV6, &policy_match_type, &audited, ext_err,
			proxy_port, &cookie);
		if (verdict == DROP_POLICY_AUTH_REQUIRED) {
		...
		if (verdict != CTX_ACT_OK || ret != CT_ESTABLISHED)
			send_policy_verdict_notify(
				ctx, src_label, tuple->dport, tuple->nexthdr,
				POLICY_INGRESS, 1, verdict, *proxy_port,
				policy_match_type, audited, auth_type, cookie);
	...
	if (ret == CT_NEW) {
		 ...
		 */
		ret = ct_create6(
			get_ct_map6(tuple), &cilium_ct_any6_global, tuple, ctx,
			CT_INGRESS, &ct_state_new, ext_err);
		if (IS_ERR(ret))
			return ret;
	}
	...
	send_trace_notify6(
		ctx, TRACE_TO_LXC, src_label, SECLABEL_IPV6, &orig_sip, LXC_ID,
		ifindex, trace.reason, trace.monitor);

	return CTX_ACT_OK;

redirect_to_proxy:
	send_trace_notify6(
		ctx, TRACE_TO_PROXY, src_label, SECLABEL_IPV6, &orig_sip,
		bpf_ntohs(*proxy_port), ifindex, trace.reason, trace.monitor);
	if (tuple_out)
		memcpy(tuple_out, tuple, sizeof(*tuple));
	return POLICY_ACT_PROXY_REDIRECT;
}

__declare_tail(CILIUM_CALL_IPV6_TO_LXC_POLICY_ONLY) static __always_inline
...
# endif
	...
	ret = ipv6_policy(
		ctx, ip6, src_label, &tuple, &ext_err, &proxy_port, from_tunnel);
	switch (ret) {
...
drop_err:
	return send_drop_notify_ext(
		ctx, src_label, SECLABEL_IPV6, LXC_ID, ret, ext_err,
		METRIC_INGRESS);
}

__declare_tail(CILIUM_CALL_IPV6_TO_ENDPOINT) int tail_ipv6_to_endpoint(
	struct __ctx_buff *ctx)
{
...
# endif
	...
	ret = ipv6_policy(
		ctx, ip6, src_sec_identity, NULL, &ext_err, &proxy_port, false);
	switch (ret) {
	case POLICY_ACT_PROXY_REDIRECT:
...
out:
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, src_sec_identity, SECLABEL_IPV6, LXC_ID, ret,
...
TAIL_CT_LOOKUP6(
	...
	CT_INGRESS, (is_defined(ENABLE_IPV4) && is_defined(ENABLE_IPV6)),
	    ...
	    bool from_tunnel)
{
	 ...
	 */
	is_untracked_fragment =
		!CONFIG(enable_ipv4_fragments) && ipfrag_is_fragment(fraginfo);
	...
	case CT_RELATED:
		...
		if (unlikely(ct_state->rev_nat_index || ct_state->nat_port)) {
			...
			ret2 = lb4_rev_nat(
				ctx, ETH_HLEN, l4_off, ct_state->rev_nat_index,
...
# if defined(ENABLE_PER_PACKET_LB)
		 ...
		 */
		if (ret == CT_NEW &&
		    ip4->saddr == CONFIG(service_loopback_ipv4).be32 &&
...
# if defined(ENABLE_NODEPORT) && defined(ENABLE_SRV6)
		ct_state_new.node_port = ct_has_nodeport_egress_entry4(
			get_ct_map4(tuple), tuple, NULL, false);
# endif /* ENABLE_NODEPORT && ENABLE_SRV6 */
		 ...
		 */
		ret = ct_create4(
			get_ct_map4(tuple), &cilium_ct_any4_global, tuple, ctx,
			CT_INGRESS, &ct_state_new, ext_err);
		if (IS_ERR(ret))
			return ret;
	}
	...
	/* Not redirected to host / proxy. */
	send_trace_notify4(
		ctx, TRACE_TO_LXC, src_label, SECLABEL_IPV4, orig_sip, LXC_ID,
		ifindex, trace.reason, trace.monitor);
...
redirect_to_proxy:
	send_trace_notify4(
		ctx, TRACE_TO_PROXY, src_label, SECLABEL_IPV4, orig_sip,
		bpf_ntohs(*proxy_port), ifindex, trace.reason, trace.monitor);
	if (tuple_out)
		*tuple_out = *tuple;
	return POLICY_ACT_PROXY_REDIRECT;
...
__declare_tail(CILIUM_CALL_IPV4_TO_LXC_POLICY_ONLY) static __always_inline
...
# endif
	...
	ret = ipv4_policy(
		ctx, ip4, src_label, &tuple, &ext_err, &proxy_port, from_tunnel);
	switch (ret) {
	case POLICY_ACT_PROXY_REDIRECT:
		if (from_tunnel)
			ctx_change_type(ctx, PACKET_HOST);
...
drop_err:
	return send_drop_notify_ext(
		ctx, src_label, SECLABEL_IPV4, LXC_ID, ret, ext_err,
		METRIC_INGRESS);
}
...
out:
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, src_sec_identity, SECLABEL_IPV4, LXC_ID, ret,
			ext_err, METRIC_INGRESS);
 ...
 */
__section_entry int cil_lxc_policy(struct __ctx_buff *ctx)
{
	...
	case bpf_htons(ETH_P_IPV6):
		if (is_defined(ENABLE_IPV4) && is_defined(ENABLE_IPV6))
			ret = tail_call_internal(
				ctx, CILIUM_CALL_IPV6_CT_INGRESS_POLICY_ONLY,
				&ext_err);
		else
	...
	case bpf_htons(ETH_P_IP):
		if (is_defined(ENABLE_IPV4) && is_defined(ENABLE_IPV6))
			ret = tail_call_internal(
				ctx, CILIUM_CALL_IPV4_CT_INGRESS_POLICY_ONLY,
				&ext_err);
		else
...
out:
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, src_label, sec_label, LXC_ID, ret, ext_err,
			METRIC_INGRESS);
 ...
 * a L7 LB.
 */
__section_entry int cil_lxc_policy_egress(struct __ctx_buff *ctx __maybe_unused)
{
#if defined(ENABLE_L7_LB)
	__be16 proto;
	int ret;
	...
	send_trace_notify(
		ctx, TRACE_FROM_PROXY, SECLABEL, UNKNOWN_ID,
		TRACE_EP_ID_UNKNOWN, TRACE_IFINDEX_UNKNOWN,
...
__section_entry int cil_to_container(struct __ctx_buff *ctx)
{
	...
	if ((ctx->mark & MARK_MAGIC_HOST_MASK) == MARK_MAGIC_PROXY_EGRESS_EPID) {
		...
		return send_drop_notify(
			ctx, lxc_id, sec_label, LXC_ID, ret, METRIC_INGRESS);
	}
#endif
	...
	send_trace_notify(
		ctx, trace, identity, sec_label, LXC_ID, ctx->ingress_ifindex,
		TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN, proto);
	...
	if (identity == HOST_ID) {
		...
		ret = tail_call_policy(ctx, CONFIG(host_ep_id));
		return send_drop_notify(
			ctx, identity, sec_label, LXC_ID, DROP_HOST_NOT_READY,
			METRIC_INGRESS);
	}
#endif /* ENABLE_HOST_FIREWALL && !ENABLE_ROUTING */
	...
	case bpf_htons(ETH_P_IPV6):
		...
		ret = tail_call_internal(
			ctx, CILIUM_CALL_IPV6_CT_INGRESS, &ext_err);
		break;
#endif /* ENABLE_IPV6 */
#ifdef ENABLE_IPV4
	case bpf_htons(ETH_P_IP):
		...
		ctx_store_meta(ctx, CB_SRC_LABEL, identity);
		ret = tail_call_internal(
			ctx, CILIUM_CALL_IPV4_CT_INGRESS, &ext_err);
		break;
	case bpf_htons(ETH_P_ARP):
		ret = CTX_ACT_OK;
		break;
...
out:
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, identity, sec_label, LXC_ID, ret, ext_err,
			METRIC_INGRESS);
...
#ifdef ENABLE_IPV4
__declare_tail(CILIUM_CALL_IPV4_POLICY_DENIED) int tail_policy_denied_ipv4(
	struct __ctx_buff *ctx)
{
	int ret;
...
#ifdef ENABLE_IPV6
__declare_tail(CILIUM_CALL_IPV6_POLICY_DENIED) int tail_policy_denied_ipv6(
	struct __ctx_buff *ctx)
{
